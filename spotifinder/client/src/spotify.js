 ...
 */
export const getRecommendationsForTracks = (tracks, artists, genres) => {
  const seed_tracks = tracks || "";
  const seed_artists = artists || "";
  const seed_genres = genres || "";

  return axios.get(
    `https://api.spotify.com/v1/recommendations?seed_tracks=${seed_tracks}&seed_artists=${seed_artists}&seed_genres=${seed_genres}&limit=60`,
    {
      headers,
    }
  );
